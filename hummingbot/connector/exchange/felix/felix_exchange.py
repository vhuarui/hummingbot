...
import hummingbot.connector.exchange.felix.felix_web_utils as web_utils
from hummingbot.connector.client_order_tracker import ClientOrderTracker
from hummingbot.connector.exchange.felix import felix_utils
from hummingbot.connector.exchange.felix.felix_api_order_book_data_source import FelixAPIOrderBookDataSource
from hummingbot.connector.exchange.felix.felix_api_user_stream_data_source import FelixAPIUserStreamDataSource
from hummingbot.connector.exchange.felix.felix_auth import FelixAuth
...
class FelixExchange(ExchangeBase):
    ...
    def __init__(self,
                 felix_api_key: str,
                 felix_api_secret: str,
                 trading_pairs: Optional[List[str]] = None,
                 trading_required: bool = True,
                 ):
        self._felix_time_synchronizer = TimeSynchronizer()
        super().__init__()
        self._trading_required = trading_required
        self._auth = FelixAuth(
            api_key=felix_api_key,
            secret_key=felix_api_secret,
            time_provider=self._felix_time_synchronizer)
        ...
        self._order_book_tracker = OrderBookTracker(
            data_source=FelixAPIOrderBookDataSource(
                trading_pairs=trading_pairs,
                api_factory=self._api_factory,
                throttler=self._throttler),
            trading_pairs=trading_pairs)
        self._user_stream_tracker = UserStreamTracker(
            data_source=FelixAPIUserStreamDataSource(
                auth=self._auth,
                throttler=self._throttler,
                api_factory=self._api_factory,
                time_synchronizer=self._felix_time_synchronizer))
    ...
    def status_dict(self) -> Dict[str, bool]:
        ...
        """
        return {
            "symbols_mapping_initialized": FelixAPIOrderBookDataSource.trading_pair_symbol_map_ready(),
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel order with Felix. Check API key and network connection."
            )
    ...
    async def _status_polling_loop(self):
        ...
        It also updates the time synchronizer. This is necessary because Felix require the time of the client to be
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.", exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Felix. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.", exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Felix. "
                                                      "Check network connection.")
                await asyncio.sleep(0.5)

    async def _update_trading_rules(self):
        symbols = await self._api_request(
            method=RESTMethod.GET,
            path_url=CONSTANTS.SYMBOLS_PATH_URL)
        trading_rules_list = await self._format_trading_rules(symbols["data"])
        self._trading_rules.clear()
        for trading_rule in trading_rules_list:
            self._trading_rules[trading_rule.trading_pair] = trading_rule

    async def _format_trading_rules(self, symbols_dict: Dict[str, Any]) -> List[TradingRule]:
        """
        Example:
        {
            "type": 1,
        ...
        """
        trading_pair_rules = symbols_dict.get("list", [])
        retval = []
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://github.com/felix-exchange/felix-official-api-docs/blob/master/user-data-stream.md
                # As per the order update section in Felix the ID of the order being cancelled is under the "C" key
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case Felix's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since Felix's get order endpoint does not return trade IDs.
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            query_time = int(self._last_trades_poll_felix_timestamp * 1e3)
            self._last_trades_poll_felix_timestamp = self._felix_time_synchronizer.time()
    ...
    async def _update_time_synchronizer(self):
        try:
            await self._felix_time_synchronizer.update_server_time_offset_with_time_provider(
            ...
            self.logger().exception("Error requesting time from Felix server")
            raise
